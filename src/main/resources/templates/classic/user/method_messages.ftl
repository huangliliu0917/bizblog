                            ...
                            <li class="list-group-item " style="padding: 0 15px;">
                                ...
                                <div class="infos">
                                    ...
                                    <#--<span class="hidden-xs label label-warning">${row.channel.name}</span>-->
                                        ...
                                        <span>
                                            <#if (row.event == 1)>
                                                收藏了你的文章 - <a href="${base}/post/${row.postId}<#if options['site_static'] == "1">.html</#if>">${row.post.title}</a>
                                            <#elseif (row.event == 3)>
                                                评论了你的文章 - <a href="${base}/post/${row.postId}<#if options['site_static'] == "1">.html</#if>">点击查看详情</a>
                                            <#elseif (row.event == 4)>
                                                回复了你的评论 - <a href="${base}/post/${row.postId}<#if options['site_static'] == "1">.html</#if>">点击查看详情</a>
                                            </#if>
                                        </span>
                                    </div>
                                </div>
