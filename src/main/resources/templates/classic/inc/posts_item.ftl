    ...
    <#if row.thumbnail?? && row.thumbnail?length gt 0>
        <div class="content-box">
            <div class="posts-item-img">
                <a href="${base}/post/${row.id}<#if options['site_static'] == "1">.html</#if>" title="">
            ...
            <div class="posts-item posts-item-gallery">
                <h2><a href="${base}/post/${row.id}<#if options['site_static'] == "1">.html</#if>"><#if escape>${row.title?html}<#else>${row.title}</#if></a></h2>
